...
void *find_max(void *arr, int n, int element_size, int (*compare)(const void *, const void *)) 
{
	void *max_elem = arr;
	int index = 1;
	void *cur_element = (char *)arr + element_size;
	while(index < n)
	{   
		int* int_max_elem = (int*)max_elem;
		int* int_cur_elem = (int*)cur_element;
		if(compare(int_max_elem, int_cur_elem) == 1)
				max_elem = cur_element;
		index++;
		cur_element = (char *)arr + index * element_size;
	}
