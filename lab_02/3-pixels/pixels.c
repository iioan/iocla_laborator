...
{
	int i = 0, j = 0, n = pic -> height;
    for (i = 0; i < n / 2; i++) 
	{
    for (j = 0; j < pic -> width; j++) 
	{
      // fac swap intre pixeli
      int r = pic -> pix_array[n - i - 1][j].R;
      int b = pic -> pix_array[n - i - 1][j].B;
      int g = pic -> pix_array[n - i - 1][j].G;
      pic -> pix_array[n - i - 1][j].R = pic -> pix_array[i][j].R;
      pic -> pix_array[n - i - 1][j].B = pic -> pix_array[i][j].B;
      pic -> pix_array[n - i - 1][j].G = pic -> pix_array[i][j].G;
      pic -> pix_array[i][j].R = r;
      pic -> pix_array[i][j].B = b;
      pic -> pix_array[i][j].G = g;
    }
  }
}
...
void colorToGray(Picture *pic)
{
    ...
    for (j = 0; j < pic -> width; j++) 
	{
      ...
      pic -> pix_array[i][j].B = 0.59 * pic -> pix_array[i][j].B;
      pic -> pix_array[i][j].G = 0.11 * pic -> pix_array[i][j].G;
    }
  }
}
